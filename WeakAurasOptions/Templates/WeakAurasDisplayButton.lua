...
local function GetDropTarget()
  ...
  for id, button in pairs(buttonList) do
    if not button.dragging and button:IsEnabled() and button:IsShown() then
      local halfHeight = button:GetHeight() / 2
      local height = button:GetHeight()
      if button.data.controlledChildren then
        if button.data.parent == nil and button:IsMouseOver(1, -1) then
          -- Top level group, always group into
          return id, button, "GROUP"
        end

        -- For sub groups, middle third is for grouping
        if button:IsMouseOver(-height / 3, height / 3) then
          return id, button, "GROUP"
        end
      end

      if button:IsMouseOver(1, height / 2) then
        return id, button, "BEFORE"
      elseif button:IsMouseOver(-height / 2, -1) then
...
local methods = {
  ...
  ["Initialize"] = function(self)
      ...
      elseif(IsShiftKeyDown()) then
        ...
        if(mouseButton == "RightButton") then
          ...
          if(OptionsPrivate.IsDisplayPicked(self.data.id) and OptionsPrivate.IsPickedMultiple()) then
            LibDD:EasyMenu(OptionsPrivate.MultipleDisplayTooltipMenu(), WeakAuras_DropDownMenu, self, 0, 0, "MENU");
          else
            UpdateClipboardMenuEntry(self.data);
            LibDD:EasyMenu(self.menu, WeakAuras_DropDownMenu, self, 0, 0, "MENU");
    ...
    self:SetScript("OnEnter", function()
      if(OptionsPrivate.IsPickedMultiple() and OptionsPrivate.IsDisplayPicked(self.id)) then
        Show_Long_Tooltip(self, OptionsPrivate.MultipleDisplayTooltipDesc());
      else
        if not self.grouping then
          self:SetNormalTooltip();
        end
        Show_Long_Tooltip(self, self.description);
      end
    end);
    self:SetScript("OnLeave", Hide_Tooltip)
    ...
    self:SetNormalTooltip()
    self:SetScript("OnClick", self.callbacks.OnClickNormal)
    self:SetScript("OnKeyDown", self.callbacks.OnKeyDown)
    self:EnableKeyboard(false)
    self:SetMovable(true)
    self:RegisterForDrag("LeftButton")
    self:SetScript("OnDragStart", self.callbacks.OnDragStart)
    self:SetScript("OnDragStop", self.callbacks.OnDragStop)
    self:SetEnable()
  ...
  ["StartGrouping"] = function(self, groupingData, selected, groupingGroup, childOfGrouping)
    ...
    self:UpdateIconsVisible()
    if(selected) then
      self:SetScript("OnClick", self.callbacks.OnClickGroupingSelf)
    ...
    elseif (childOfGrouping) then
      ...
      elseif (self.data.regionType == "group" or self.data.regionType == "dynamicgroup") then
        self:SetScript("OnClick", self.callbacks.OnClickGrouping);
  ...
  ["DragStart"] = function(self, mode, picked, mainAura, size)
    ...
    if picked then
      ...
      local uiscale, scale = UIParent:GetScale(), self:GetEffectiveScale()
      local x, w = self:GetLeft(), self:GetWidth()
      ...
      if self.data.id == mainAura.id then
        self:SetPoint("Center", UIParent, "BOTTOMLEFT", (x+w/2)*scale/uiscale, y/uiscale)
        if mode == "MULTI" then
          -- change label & icon
          self:SetPoint("Center", UIParent, "BOTTOMLEFT", (x+w/2)*scale/uiscale, y/uiscale)
          self.temp.title = self.title:GetText()
  ...
  ["DropEnd"] = function(self)
    ...
    self:SetScript("OnClick", self.callbacks.OnClickNormal)
    self:EnableKeyboard(false); -- disables self.callbacks.OnKeyDown
    self.view:Show()
    ...
    if self.dragging then
      if self.temp.title then
        -- restore title and icon
        self.title:SetText(self.temp.title)
        self:RestoreIcon();
      end
      self:SetParent(self.temp.parent)
      self:SetFrameStrata(self.temp.strata)
      self.temp = nil
    end
    self.dragging = false
    self:UpdateIconsVisible()
  ...
  ["UpdateUpDownButtons"] = function(self)
    if self.first or not self:IsEnabled() then
      ...
      self.upgroup:GetNormalTexture():SetVertexColor(0.3, 0.3, 0.3)
    else
      self.upgroup:Enable()
      self.upgroup:GetNormalTexture():SetVertexColor(1, 1, 1)
    end

    if self.last or not self:IsEnabled() then
      self.downgroup:Disable()
      self.downgroup:GetNormalTexture():SetVertexColor(0.3, 0.3, 0.3)
    else
      self.downgroup:Enable()
      self.downgroup:GetNormalTexture():SetVertexColor(1, 1, 1)
    end
  end,
  ["GetGroupOrder"] = function(self)
    return self.dgrouporder
  end,
  ...
  ["UpdateViewTexture"] = function(self)
    ...
    if(visibility == 2) then
      self.view:GetNormalTexture():SetTexture("Interface\\LFGFrame\\BattlenetWorking0.blp")
    elseif(visibility == 1) then
      self.view:GetNormalTexture():SetTexture("Interface\\LFGFrame\\BattlenetWorking2.blp")
    else
      self.view:GetNormalTexture():SetTexture("Interface\\LFGFrame\\BattlenetWorking4.blp")
    end
  end,
  ["GetVisibility"] = function(self)
    return self.view.visibility;
  end,
  ["SetDisable"] = function(self)
  ...
  ["UpdateThumbnail"] = function(self)
    ...
    if self.data.regionType ~= self.thumbnailType then
      ...
      if option and option.modifyThumbnail then
        option.modifyThumbnail(self, self.thumbnail, self.data)
      end
    end
  end,
  ...
  ["AcquireThumbnail"] = function(self)
    ...
    if option and option.acquireThumbnail then
      self.thumbnail = option.acquireThumbnail(self, self.data)
...
function WeakAurasDisplayButtonMixin:Init(node)
  ...
  self.view:SetScript("OnEnter", function() Show_Tooltip(self, L["View"], L["Toggle the visibility of this display"]) end);
  self.view:SetScript("OnLeave", Hide_Tooltip)
  self.view.visibility = 0

  self.group:SetScript("OnEnter", function() Show_Tooltip(self, L["Group (verb)"], L["Put this display in a group"]) end);
  self.group:SetScript("OnLeave", Hide_Tooltip)

  self.ungroup:SetScript("OnEnter", function() Show_Tooltip(self, L["Ungroup"], L["Remove this display from its group"]) end);
  self.ungroup:SetScript("OnLeave", Hide_Tooltip)

  self.upgroup:SetScript("OnEnter", function() Show_Tooltip(self, L["Move Up"], L["Move this display up in its group's order"]) end);
  self.upgroup:SetScript("OnLeave", Hide_Tooltip)

  self.downgroup:SetScript("OnEnter", function()
    Show_Tooltip(self, L["Move Down"], L["Move this display down in its group's order"])
  end)
