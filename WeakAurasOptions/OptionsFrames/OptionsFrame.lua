...
function OptionsPrivate.CreateFrame()
  ...
  buttonsScroll.ttsSetFocus = function()
        ...
        for index, button in ipairs(buttonsScroll.children) do
          if button.frame.id == frame.pickedDisplay then
            ...
            if toPick then
              ...
              -- TODO probably play a error beep?
            end
            OptionsPrivate.TTSFocus:ReadFocus()
            break;
          end
  ...
  buttonsScroll.ttsSkipFocus = function() return false end
  ...
  frame.FillOptions = function(self)
    ...
    -- TODO Check whether any widget that we are going to release has focus, if so
    -- we need to find it again, to give it focus
    ...
    -- TODO This is hacky and needs to be rewritten, we shouldn't pollute AceGui widgets
    -- with our next/prev focus information
