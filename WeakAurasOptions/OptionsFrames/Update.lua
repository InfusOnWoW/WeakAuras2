...
local function BuildUidMap(data, children, type)
  ...
  uidMap.GetDiff = function(self, uid, categories)
    ...
    local diff = CopyTable(self.map[uid].diff)
  ...
  uidMap.EnsureUniqueIdOfUnmatched = function(self, uid)
    uid = uid or self.root
    if not self.map[uid] then
      error("EnsureUniqueIdOfUnmatched for unknown uid")
      return
    end
    ...
    for _, childUid in ipairs(children) do
      self:EnsureUniqueIdOfUnmatched(childUid)
    end
  end

  uidMap.InsertUnmatchedPhase1 = function(self, otherUidMap, otherUid)
    ...
    for index, childUid in ipairs(children) do
      local needsToBeInserted = self:InsertUnmatchedPhase1(otherUidMap, childUid)
  ...
  uidMap.InsertUnmatchedFrom = function(self, otherUidMap)
    self:InsertUnmatchedPhase1(otherUidMap, otherUidMap:GetRootUID())
  end
...
local methods = {
  Open = function(self, data, children, target) -- TODO sender information, old "import" parameter
    ...
    self:AddBasicInformationWidgets(data)
  ...
  ImportImpl = function(self)
    ...
    -- Adjust UI
    self:ReleaseChildren()
    self:AddBasicInformationWidgets(pendingData.data)
    ...
    elseif userChoices.mode == "update" then
      -- TODO correct progress calculation
      self:InitializeProgress((matchInfo.oldUidMap:GetTotalCount() + matchInfo.newUidMap:GetTotalCount()) * 3 + 1)
      ...
      matchInfo.newUidMap:EnsureUniqueIdOfUnmatched()
      self:IncProgress()
      coroutine.yield()
      ...
      local GetPhase2Data
      local ChangeId -- For renaming auras
      if userChoices.activeCategories.arrangement then
        ...
        if not userChoices.activeCategories.oldchildren then
          -- Keep old children
          matchInfo.newUidMap:InsertUnmatchedFrom(matchInfo.oldUidMap)
        end
        ...
        if userChoices.activeCategories.newchildren then
          -- Add new children
          matchInfo.oldUidMap:InsertUnmatchedFrom(matchInfo.newUidMap)
        end
  ...
  ImportPhase1 = function(self, uidMap, uid, phase2Order)
    ...
    for index, childUid in ipairs(children) do
      ...
      uidMap:SetGroupOrder(childUid, index, #children)
  ...
  AddBasicInformationWidgets = function(self, data)
